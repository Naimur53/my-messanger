    ...
    async function join() {
      ...
      if (!isLoading && email !== undefined && email === client?.data?.email) {
        const rooms =
          ...
          parseInt(client?.data?.createdAt);
        setRoom(rooms);
  ...
  return (
    <div className="col-span-2 h-screen px-2">
      <div className="h-full flex flex-col   ">
        ...
        <Box
          sx={{ flexGrow: "0" }}
          className="rows-span   mt-10 bg-primary  w-full   flex "
        >
          <form
          ...
          >
            <div className="relative">
              <label
                htmlFor="files"
                className="btn bg-white p-2 rounded-full inline-flex justify-center"
              >
                <ImageIcon></ImageIcon>
              </label>
              <input
            ...
            <input
              ...
              {...register("message", {
                required: watch("pic")?.length ? false : true,
              })}
            />
            <div>
              {imgLoading ? (
